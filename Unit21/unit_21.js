...
function t2(event) {
    sum = sum + 1;
    document.querySelector('.out-2').innerHTML = sum;
    console.log(event);
}
...
//         out.innerHTML = 1;
//     }
//     else if (event.target.innerHTML = '3-2') {
//         out.innerHTML = 2;
//     }
    ...
    handleEvent(e) {
      ...
      if (block) {
        ...
        document.querySelector(
          ".out-3"
        ).textContent = `блок: ${block.classList[0]}, счётчик: ${this.counter}`;
      }
    }
  });
...
document.querySelector('.b-4').onclick = () => document.querySelector('.div-4').addEventListener('touchstart', t4);
...
// Task 8 ============================================
/*  Дан блок div-8. Добавьте на него событие touch, которое при срабатывании окрашивает 
блок случаным цветом из массива a8=[red, green, blue, orange, pink, yellow] */

function random(min, max) { return Math.floor(Math.random() * (max - min + 1) + min); }

function t8() {
    let a8 = ['red', 'green', 'blue', 'orange', 'pink', 'yellow'];
    let rand = a8[random(0, a8.length - 1)];
...
let p = 70;
function t10() {
    document.querySelector('.div-10').style.width = p + 'px';
    p++;
}
...
function t11(event) {
    console.log(event);
    document.querySelector('.out-11').innerHTML = `radiusY:${event.radiusY} radiusX: ${event.radiusX}`;
}
